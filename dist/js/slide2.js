document.addEventListener('DOMContentLoaded', () => {
  ...
  (function(){
    if (typeof gsap === 'undefined') {
      console.error('GSAP non chargé. Assure-toi d\'inclure gsap avant ce script.');
      return;
    }

    const leafCount = 30;
    const container = document.getElementById('leaves');
    if (!container) { console.error('#leaves manquant'); return; }
    ...
    function startFall(leaf, delay = 0) {
      ...
      leaf._fallTween = gsap.to(leaf, {
        ...
        onComplete: () => {
          ...
          gsap.set(leaf, { x: 0, y: newStartY, rotation: rand(-10,10), opacity: 1 });
